...
SELECT '=== 单元数据 ===' as info;
SELECT u.id, u.building_id, b.building_name, u.unit_name, u.unit_no, u.total_floors, u.total_rooms 
FROM unit u 
LEFT JOIN building b ON u.building_id = b.id 
WHERE u.deleted = 0 
ORDER BY u.building_id, u.id;

SELECT '=== 房屋数据 ===' as info;
SELECT r.id, r.building_id, b.building_name, r.unit_id, u.unit_name, r.room_no, r.floor, r.room_type, r.area, r.status, r.property_fee
FROM room r
